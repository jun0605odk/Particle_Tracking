function dd = MSD_4(trackdata,unit,im,disp_move)
...
for (particleid=1:max(trackdata(:,6)))
    ...
    if ( totalframe > 10 && dd(index+totalframe-1,2)>0)
        ...
        X2_2{c_over10}(:) = trackdata(index:index+totalframe-1,1)*unit(2) -...
                            trackdata(index,1)*unit(2);
        Y2_2{c_over10}(:) = trackdata(index:index+totalframe-1,2)*unit(2) -...
                            trackdata(index,2)*unit(2);
