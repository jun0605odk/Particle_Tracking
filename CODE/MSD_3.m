function dd = MSD_3(trackdata,im,no_display)
...
dd(:,4) = trackdata(:,6);%!
MFI = zeros(size(max(trackdata(:,6))));
...
for (particleid=1:max(trackdata(:,6)))
    ...
    % Caluculate 
    MFI(particleid) = sum(trackdata(index:index+totalframe-1,3)) / totalframe;%! MFI(mean fluorescence intensity)
    ...
    if ( totalframe > 10 && dd(index+totalframe-1,2)>0)
        ...
        str2 = sprintf('   %s %7.2f','MFI:',MFI(particleid));%! MFI(mean fluorescence intensity)
...
%if no_display~=1
    ...
    histogram(MFI);
    axis([0 3000 0 1000])
    title ('MFI','FontSize',16);
    xlabel('MFI');
    ylabel('frequency');
%end

MFI_all = sum(MFI) / max(trackdata(:,6));
str4 = sprintf('%s %7.2f','MFI (all)',MFI_all);
disp([str4])
